...
void add_product_ui(Console* console)
{
        ...
        && validate_product_type(type))
    {
        ...
        add_product_service(&console->service, name, type, new_quantity, date);
    }
    else
...
void update_products_expiration_date_ui(Console* console)
{
    ...
    {
        if (update_products_expiration_date_service(&console->service, name, type, date) == 1)
...
void update_products_quantity_ui(Console* console)
{
    ...
    {
        new_quantity = validate_product_quantity(quantity);
        if (update_products_quantity_service(&console->service, name, type, new_quantity) == 1)
...
void search_elements_by_string_ui(Console* console)
{
    ...
    {
        search_elements_by_string_service(&console->service, string, Repository, criteria);
        for (int i = 0; i < Repository->array.count; i++)
            printf("Name: %s  Type: %s  Quantity: %d  Expiration date: %s\n", Repository->array.data[i].name,
                Repository->array.data[i].type, Repository->array.data[i].quantity, Repository->array.data[i].date);
    }
...
void display_products_by_category(Console* console)
{
    ...
    if (validate_product_type(type) != 0)
    {
        ...
        if (search_elements_by_category_service(&console->service, type, Repo, number_of_days) == 0)
            printf("There are no products which check the conditions for display!");
        else if (search_elements_by_category_service(&console->service, type, Repo, number_of_days) == 1)
        {
            ...
            for (int i = 0; i < Repo->array.count; i++)
                printf("Name: %s  Type: %s  Quantity: %d  Expiration date: %s\n", Repo->array.data[i].name,
                    Repo->array.data[i].type, Repo->array.data[i].quantity, Repo->array.data[i].date);
